...
def calcEstimate(filename, websites):
	...
	for website in websites:
		...
		if sponsoredCount != 0:
			...
			# consider special case where there is a 1/1 website, we don't want facebook, twitter,
			# and other valid websites to pull down the average and thus decrease the score
			if ratio >= 0.5:
				average = 1
				break
		l.append(ratio)
	if average == 1:
		return [filename, average] #return our special case
	if len(l) != 0:
		average = sum(l)/len(l)
...
sponsoredDict = pickle.load(open( "sponsoredDict.p", "rb"))
notsponsoredDict = pickle.load(open( "notsponsoredDict.p", "rb"))
sampleDict = pickle.load(open( "sampleDict.p", "rb"))
